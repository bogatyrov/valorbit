...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("valorbit"))
        return false;
...
{
    // Convert valorbit:// to valorbit:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("valorbit://"))
    {
        uri.replace(0, 12, "valorbit:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Valorbit.lnk";
}
...
{
    return GetAutostartDir() / "valorbit.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Valorbit-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  valorbit-qt [" + tr("command-line options") + "]                     " + "\n";
