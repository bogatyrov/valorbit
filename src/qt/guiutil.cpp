...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("valorcoin"))
        return false;
...
{
    // Convert valorcoin:// to valorcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("valorcoin://"))
    {
        uri.replace(0, 12, "valorcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ValorCoin.lnk";
}
...
{
    return GetAutostartDir() / "valorcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("ValorCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  valorcoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("ValorCoin-Qt"));
...
void SetValorThemeQSS(QApplication& app)
{
